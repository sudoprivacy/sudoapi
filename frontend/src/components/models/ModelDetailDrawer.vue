...
<template>
  <Teleport to="body">
    <transition name="fade">
      <div v-if="open" class="fixed inset-0 z-[99998] bg-black/40 backdrop-blur-sm" @click="close" />
    </transition>
        ...
        <div class="space-y-6 px-5 py-5">
          ...
          <section>
            <h3 class="mb-2 text-sm font-semibold text-gray-900 dark:text-white">
              {{ t('modelCatalog.detail.basicInfo') }}
            </h3>
            <p class="mb-3 text-sm text-gray-600 dark:text-dark-300">
              {{ card.description || t('modelCatalog.detail.noDescription') }}
            </p>
            <dl class="grid grid-cols-2 gap-3 text-xs sm:grid-cols-3">
              <div v-if="card.context_window > 0">
                <dt class="text-gray-500 dark:text-dark-400">
                  {{ t('modelCatalog.detail.contextWindow') }}
                </dt>
              ...
              <div v-if="card.max_output > 0">
                <dt class="text-gray-500 dark:text-dark-400">
                  {{ t('modelCatalog.detail.maxOutput') }}
                </dt>
                <dd class="font-medium text-gray-900 dark:text-white">
              ...
              <div>
                <dt class="text-gray-500 dark:text-dark-400">
                  {{ t('modelCatalog.detail.category') }}
                </dt>
                <dd class="font-medium text-gray-900 dark:text-white">
                  {{ t(`modelCatalog.categories.${card.category}`, card.category) }}
                </dd>
              </div>
              <div v-if="card.model_type">
                <dt class="text-gray-500 dark:text-dark-400">
                  {{ t('modelCatalog.detail.modelType') }}
                </dt>
                <dd class="font-medium text-gray-900 dark:text-white">
              ...
              <div v-if="card.input_modalities && card.input_modalities.length">
                <dt class="text-gray-500 dark:text-dark-400">
                  {{ t('modelCatalog.detail.inputModalities') }}
                </dt>
                <dd class="font-medium text-gray-900 dark:text-white">
              ...
              <div v-if="card.output_modalities && card.output_modalities.length">
                <dt class="text-gray-500 dark:text-dark-400">
                  {{ t('modelCatalog.detail.outputModalities') }}
                </dt>
                <dd class="font-medium text-gray-900 dark:text-white">
          ...
          <section>
            <h3 class="mb-2 text-sm font-semibold text-gray-900 dark:text-white">
              {{ t('modelCatalog.detail.endpoints') }}
            </h3>
            <p class="mb-3 text-xs text-gray-500 dark:text-dark-400">
              {{ t('modelCatalog.detail.endpointsHint') }}
            </p>
            <div class="space-y-3">
              <div class="rounded-lg border border-gray-200 p-3 dark:border-dark-700">
                <ul class="space-y-1.5">
                  ...
                  >
                    <span
                      class="rounded bg-green-500/10 px-1.5 py-0.5 text-[10px] font-semibold text-green-700 dark:text-green-300"
                    >
                      {{ ep.method }}
                    </span>
          ...
          <section>
            <h3 class="mb-2 text-sm font-semibold text-gray-900 dark:text-white">
              {{ t('modelCatalog.detail.groupPrices') }}
            </h3>
            <p class="mb-3 text-xs text-gray-500 dark:text-dark-400">
              {{ t('modelCatalog.detail.groupPricesHint') }}
            </p>
            <div class="space-y-4">
              <div v-for="platform in card.platforms" :key="platform.platform">
                <div v-if="!platform.group_prices.length" class="text-xs text-gray-500 dark:text-dark-400">
                  {{ t('modelCatalog.detail.noPricing') }}
                </div>
                <div
                  v-else
                ...
                >
                  ...
                  >
                    <span class="font-medium">{{ t('modelCatalog.detail.callChain', { group: row.group_name }) }}</span>
                  ...
                  <table class="w-full text-xs">
                    <thead>
                      <tr
                        class="border-b border-gray-100 text-left text-[11px] uppercase text-gray-400 dark:border-dark-700"
                      >
                        <th class="py-1">{{ t('modelCatalog.detail.priceItem') }}</th>
                        <th class="py-1 text-right">{{ t('modelCatalog.detail.priceValue') }}</th>
                      </tr>
                    </thead>
                    <tbody>
                      <PricingRow
                        v-if="row.billing_mode === 'token'"
                        :label="t('modelCatalog.detail.inputPrice')"
                      ...
                      <PricingRow
                        v-if="row.billing_mode === 'token'"
                        :label="t('modelCatalog.detail.outputPrice')"
                      ...
                      <PricingRow
                        ...
                        :label="t('modelCatalog.detail.cacheReadPrice')"
                      ...
                      <PricingRow
                        ...
                        :label="t('modelCatalog.detail.cacheWritePrice')"
                      ...
                      <PricingRow
                        ...
                        :label="t('modelCatalog.detail.cacheCreation5mPrice')"
                      ...
                      <PricingRow
                        ...
                        :label="t('modelCatalog.detail.cacheCreation1hPrice')"
                      ...
                      <PricingRow
                        ...
                        :label="t('modelCatalog.detail.imageOutputPrice')"
                      ...
                      <PricingRow
                        ...
                        :label="t('modelCatalog.detail.perRequestPrice')"
                  ...
                  >
                    <div
                      class="border-b border-gray-100 px-2 py-1.5 text-[11px] font-medium text-gray-500 dark:border-dark-700 dark:text-dark-300"
                    >
                      {{ t('modelCatalog.detail.intervalPrices') }}
                    </div>
                    <table v-if="row.billing_mode === 'token'" class="min-w-[720px] w-full text-xs">
                      <thead>
                        <tr class="border-b border-gray-100 text-left text-[11px] text-gray-400 dark:border-dark-700">
                          <th class="px-2 py-1">{{ t('modelCatalog.detail.contextRange') }}</th>
                          <th class="px-2 py-1 text-right">{{ t('modelCatalog.detail.inputPrice') }}</th>
                          <th class="px-2 py-1 text-right">{{ t('modelCatalog.detail.outputPrice') }}</th>
                          <th class="px-2 py-1 text-right">{{ t('modelCatalog.detail.cacheReadPrice') }}</th>
                          <th class="px-2 py-1 text-right">{{ t('modelCatalog.detail.cacheWritePrice') }}</th>
                          <th v-if="hasIntervalCacheCreation5m(row)" class="px-2 py-1 text-right">
                            {{ t('modelCatalog.detail.cacheCreation5mPrice') }}
                          </th>
                          <th v-if="hasIntervalCacheCreation1h(row)" class="px-2 py-1 text-right">
                            {{ t('modelCatalog.detail.cacheCreation1hPrice') }}
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        ...
                        >
                          ...
                          <td class="px-2 py-1.5 text-right font-mono text-gray-900 dark:text-white">
                            <ModelPriceValue
                              :value="iv.input_price_per_mtok_usd"
                              :mult="effectiveMultiplier(row)"
                              unit="MTok"
                            />
                          </td>
                          <td class="px-2 py-1.5 text-right font-mono text-gray-900 dark:text-white">
                            <ModelPriceValue
                              :value="iv.output_price_per_mtok_usd"
                              :mult="effectiveMultiplier(row)"
                              unit="MTok"
                            />
                          </td>
                          <td class="px-2 py-1.5 text-right font-mono text-gray-900 dark:text-white">
                            <ModelPriceValue
                              :value="iv.cache_read_price_per_mtok_usd"
                              :mult="effectiveMultiplier(row)"
                              unit="MTok"
                            />
                          </td>
                          <td class="px-2 py-1.5 text-right font-mono text-gray-900 dark:text-white">
                            <ModelPriceValue
                              :value="iv.cache_write_price_per_mtok_usd"
                              :mult="effectiveMultiplier(row)"
                              unit="MTok"
                            />
                          </td>
                          <td
                            v-if="hasIntervalCacheCreation5m(row)"
                            class="px-2 py-1.5 text-right font-mono text-gray-900 dark:text-white"
                          >
                            <ModelPriceValue
                              :value="iv.cache_creation_5m_price_per_mtok_usd"
                              :mult="effectiveMultiplier(row)"
                              unit="MTok"
                            />
                          </td>
                          <td
                            v-if="hasIntervalCacheCreation1h(row)"
                            class="px-2 py-1.5 text-right font-mono text-gray-900 dark:text-white"
                          >
                            <ModelPriceValue
                              :value="iv.cache_creation_1h_price_per_mtok_usd"
                              :mult="effectiveMultiplier(row)"
                              unit="MTok"
                            />
                          </td>
                    ...
                    <table v-else class="min-w-[420px] w-full text-xs">
                      <thead>
                        <tr class="border-b border-gray-100 text-left text-[11px] text-gray-400 dark:border-dark-700">
                          <th class="px-2 py-1">{{ t('modelCatalog.detail.tier') }}</th>
                          <th class="px-2 py-1">{{ t('modelCatalog.detail.contextRange') }}</th>
                          <th class="px-2 py-1 text-right">{{ t('modelCatalog.detail.perRequestPrice') }}</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr
                        ...
                        >
                          ...
                          <td class="px-2 py-1.5 text-right font-mono text-gray-900 dark:text-white">
                            <ModelPriceValue
                              :value="iv.per_request_price_usd"
                              :mult="effectiveMultiplier(row)"
                              unit="call"
                            />
                          </td>
...
import type { ModelCatalogCard, ModelGroupPrice, ModelPriceInterval } from '@/api/models'
...
function modalityLabel(key: string): string {
  return t(`modelCatalog.modalities.${key}`, humanizeKey(key))
}
...
function discountLabel(row: ModelGroupPrice): string {
  ...
  return t('modelCatalog.detail.discountValue', { fold: formatDiscountFold(mult) })
}
