...
<template>
  ...
  <span v-else class="inline-flex flex-wrap items-baseline justify-end gap-x-1.5 gap-y-0.5">
    ...
    <span v-if="showOriginal" class="whitespace-nowrap text-[10px] text-gray-400 line-through dark:text-dark-500">
      ${{ formattedOriginal }}
    </span>
    <span class="whitespace-nowrap text-[10px] text-gray-400 dark:text-dark-500"> / {{ unit }} </span>
  </span>
</template>
