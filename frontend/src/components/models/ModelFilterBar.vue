...
const categoryOptions = computed(() =>
  props.availableCategories.map((c) => ({
    value: c,
    label: t(`modelCatalog.categories.${c}`, c),
  }))
)
const capabilityOptions = computed(() =>
  props.availableCapabilities.map((c) => ({
    value: c,
    label: t(`modelCatalog.capabilities.${c}`, humanizeKey(c)),
  }))
)
const priceRangeOptions = computed(() => [
  { value: 'free', label: t('modelCatalog.priceTier.free') },
  { value: 'low', label: t('modelCatalog.priceTier.low') },
  { value: 'mid', label: t('modelCatalog.priceTier.mid') },
  { value: 'high', label: t('modelCatalog.priceTier.high') },
])
