<template>
  <div class="space-y-5">
    ...
    <div
      class="flex flex-wrap items-center justify-between gap-3 text-xs text-gray-500 dark:text-dark-400"
    >
      <span>
        {{
        ...
        <label v-if="!compact" class="flex items-center gap-1.5">
          ...
          <select v-model="sortKey" class="input h-7 py-0 pl-2 pr-7 text-xs">
            <option value="featured">
              {{ t("modelSquare.sort.featured") }}
            </option>
            <option value="nameAsc">{{ t("modelSquare.sort.nameAsc") }}</option>
            <option value="nameDesc">
              {{ t("modelSquare.sort.nameDesc") }}
            </option>
            <option value="priceAsc">
              {{ t("modelSquare.sort.priceAsc") }}
            </option>
            <option value="priceDesc">
              {{ t("modelSquare.sort.priceDesc") }}
            </option>
            <option value="contextDesc">
              {{ t("modelSquare.sort.contextDesc") }}
            </option>
          </select>
        </label>
        <button
    ...
    >
      ...
      <button
        type="button"
        @click="reload"
        class="btn btn-secondary h-7 px-3 text-xs"
      >
    ...
    <!-- Loading skeleton -->
    <div
      v-else-if="loading && !allCards.length"
...
const sortKey = ref<
  "featured" | "nameAsc" | "nameDesc" | "priceAsc" | "priceDesc" | "contextDesc"
>("featured");
...
const availableCapabilities = computed(() => {
  ...
  for (const c of allCards.value)
    cardSupportFlags(c).forEach((cap) => set.add(cap));
  return Array.from(set).sort();
});
...
const filteredCards = computed(() => {
  ...
  return allCards.value.filter((c) => {
    ...
    if (filterState.value.capabilities.length) {
      ...
      if (!filterState.value.capabilities.every((cap) => caps.has(cap)))
        return false;
    }
...
const displayedCards = computed(() =>
  props.compact
    ? visibleCards.value.slice(0, props.maxItems)
    : visibleCards.value,
);
