        ...
        >
          ...
          <template #cell-review_status="{ row }">
            <div class="flex flex-col items-start gap-1">
              ...
              <span v-if="row.owner_user_id" class="text-xs text-gray-500 dark:text-dark-400">
                {{ t('admin.accounts.externalSubmission') }}: {{ ownerUserLabel(row) }}
              </span>
            </div>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button v-if="row.owner_user_id && row.review_status === 'pending'" @click="handleReview(row, 'approved')" :disabled="reviewingId === row.id" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-emerald-50 hover:text-emerald-600 dark:hover:bg-emerald-900/20 dark:hover:text-emerald-400">
                <Icon name="check" size="sm" />
                <span class="text-xs">{{ t('admin.accounts.approve') }}</span>
              </button>
              <button v-if="row.owner_user_id && row.review_status === 'pending'" @click="handleReview(row, 'rejected')" :disabled="reviewingId === row.id" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-amber-50 hover:text-amber-600 dark:hover:bg-amber-900/20 dark:hover:text-amber-400">
                <Icon name="x" size="sm" />
...
function ownerUserLabel(row: Account) {
  const owner = row.owner_user
  return owner?.username || owner?.email || (row.owner_user_id ? `#${row.owner_user_id}` : '-')
}
...
const handleExportData = async () => {
  ...
  try {
    ...
    const filename = `sudorouter-account-${timestamp}.json`
