export default {
  ...
  home: {
    ...
    goToDashboard: "进入控制台",
    // 新增：面向用户的价值主张
    heroSubtitle: "统一接入多种 AI 能力",
    heroDescription:
      "为团队和应用提供稳定、可观测的模型访问入口，简化密钥、计费与路由管理",
    tags: {
      subscriptionToApi: "统一网关",
      stickySession: "会话保持",
    ...
    features: {
      unifiedGateway: "一键接入",
      unifiedGatewayDesc:
        "通过统一入口接入已配置的 AI 模型，降低多平台对接和维护成本。",
      multiAccount: "稳定可靠",
      multiAccountDesc:
        "根据可用性和策略自动选择上游通道，减少单点故障对业务的影响。",
      balanceQuota: "用多少付多少",
      balanceQuotaDesc:
  ...
  // Setup Wizard
  setup: {
    title: "SudoRouter 安装向导",
    description: "配置您的 SudoRouter 实例",
    database: {
      title: "数据库配置",
  ...
  liteLLMModels: {
    title: "模型广场",
    subtitle: "浏览模型目录、能力信息与官方参考价",
    searchPlaceholder: "搜索模型、厂商、类型、能力...",
    empty: "没有符合条件的模型",
    resultCount: "显示 {count} / {total} 个模型",
    lastLoaded: "加载于 {time}",
    loadFailed: "模型列表加载失败：{msg}",
    filters: {
      allProviders: "全部厂商",
      allModes: "全部类型",
    },
    ...
    detail: {
      ...
      pricingRules: "计费规则",
      inputPrice: "输入价格",
      inputPriorityPrice: "优先级输入价格",
      outputPrice: "输出价格",
      outputPriorityPrice: "优先级输出价格",
      cacheWritePrice: "缓存写入 5m",
      cacheWrite1hPrice: "缓存写入 1h",
      cacheReadPrice: "缓存读取",
      cacheReadPriorityPrice: "优先级缓存读取",
      imagePrice: "图片价格",
      imageTokenPrice: "图片输出 Token",
    ...
    pricingSections: {
      token: "Token 计费",
      cache: "缓存计费",
      serviceTier: "优先级 / Flex 计费",
      multimodal: "多模态计费",
      batchAndText: "批量、字符与搜索计费",
      longContext: "长上下文规则",
    },
    pricingLabels: {
      inputAbove128k: "输入价格（超过 128K token）",
      outputAbove128k: "输出价格（超过 128K token）",
      inputAbove200k: "输入价格（超过 200K token）",
      outputAbove200k: "输出价格（超过 200K token）",
      cacheWriteAbove200k: "缓存写入（超过 200K token）",
      cacheReadAbove200k: "缓存读取（超过 200K token）",
      audioCacheWrite: "音频缓存写入",
      audioCacheRead: "音频缓存读取",
      imageCacheRead: "图像缓存读取",
      priorityAudioInput: "优先级音频输入",
      priorityInputAbove200k: "优先级输入（超过 200K token）",
      priorityOutputAbove200k: "优先级输出（超过 200K token）",
      priorityCacheReadAbove200k: "优先级缓存读取（超过 200K token）",
      flexInput: "Flex 输入",
      flexOutput: "Flex 输出",
      flexCacheRead: "Flex 缓存读取",
      imageInput: "图像输入",
      imageInputAbove128k: "图像输入（超过 128K token）",
      imageInputToken: "图像输入 token",
      audioInputToken: "音频输入 token",
      audioOutputToken: "音频输出 token",
      audioInputSecond: "音频输入时长",
      audioInputSecondAbove128k: "音频输入时长（超过 128K token）",
      videoInputSecond: "视频输入时长",
      videoInputSecondAbove128k: "视频输入时长（超过 128K token）",
      outputSecond: "输出时长",
      batchInput: "批量输入",
      batchOutput: "批量输出",
      characterInput: "字符输入",
      characterOutput: "字符输出",
      characterInputAbove128k: "字符输入（超过 128K token）",
      characterOutputAbove128k: "字符输出（超过 128K token）",
      searchContext: "搜索上下文查询",
    },
  },
  ...
  admin: {
    ...
    backup: {
      ...
      r2Guide: {
        ...
        step1: {
          ...
          line2:
            "点击「创建存储桶」，输入名称（如 sudorouter-backups），选择区域",
          line3: "点击创建完成",
        },
        step2: {
          title: "创建 API 令牌",
    ...
    users: {
      ...
      batch: {
        ...
        columnsHint:
          "支持逗号 / 分号 / 制表符分隔；可带或不带表头；# 开头的行被忽略",
        securityNotice:
          "请在受信网络下操作。CSV 中包含明文密码，提交后不会被记录到日志。",
        uploadCsv: "上传 CSV",
    ...
    modelMetadata: {
      ...
      form: {
        ...
        supportFlagsHint:
          "模型标签来自 LiteLLM 中所有为 true 的 supports_* 字段；不选则沿用自动数据。",
    ...
    accounts: {
      ...
      dataImportResultSummary:
        "代理创建 {proxy_created}，复用 {proxy_reused}，失败 {proxy_failed}；账号创建 {account_created}，跳过 {account_skipped}，失败 {account_failed}",
      dataImportErrors: "失败详情",
      dataImportSuccess:
        "导入完成：账号创建 {account_created}，跳过 {account_skipped}，失败 {account_failed}",
      dataImportCompletedWithErrors:
        "导入完成但有错误：账号跳过 {account_skipped}，账号失败 {account_failed}，代理失败 {proxy_failed}",
    ...
    settings: {
      ...
      linuxdo: {
        ...
        description: "配置 LinuxDo Connect OAuth，用于 SudoRouter 用户登录",
      ...
      site: {
        ...
        siteSubtitlePlaceholder: "稳定的 AI 统一网关",
        apiBaseUrl: "API 端点地址",
        apiBaseUrlHint:
      ...
      payment: {
        ...
        field_fuiouApiBaseHint:
          "生产环境填写 https://hlwnets.fuioupay.com，沙箱/测试环境填写 https://hlwnets-test.fuioupay.com。请确保 API Key 与环境一致。",
        fuiouGuideSummary:
          "富友聚合支付：通过同一个商户号同时受理支付宝和微信支付。",
        fuiouGuideNote:
          "商户私钥需 PKCS8 + Base64 编码；富友公钥为对接方在富友后台下载的 PKIX + Base64 公钥；商户号 mchnt_cd 同时用于回调归属，请确保与富友合同/后台一致。",
  ...
  onboarding: {
    ...
    user: {
      welcome: {
        title: "👋 欢迎使用 SudoRouter",
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">您好！欢迎来到 SudoRouter AI 服务平台。</p><p style="margin-bottom: 12px;"><b>🎯 快速开始：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>🔑 创建 API 密钥</li><li>📋 复制密钥到您的应用</li><li>🚀 开始使用 AI 服务</li></ul><p style="color: #10b981; font-weight: 600;">只需 1 分钟，让我们开始吧 →</p></div>',
        nextBtn: "开始 🚀",
        prevBtn: "跳过",
      },
      keyManage: {
