	...
	result, err := r.db.ExecContext(ctx,
		`UPDATE channel_model_pricing
		 SET models = $1, billing_mode = $2, input_price = $3, output_price = $4, cache_write_price = $5, cache_read_price = $6, image_input_price = $7, image_output_price = $8, per_request_price = $9, time_pricing = $10, platform = $11, updated_at = NOW(),
		  cache_creation_5m_price = $12, cache_creation_1h_price = $13
		 WHERE id = $14`,
		modelsJSON, billingMode, pricing.InputPrice, pricing.OutputPrice, pricing.CacheWritePrice, pricing.CacheReadPrice,
		pricing.ImageInputPrice, pricing.ImageOutputPrice, pricing.PerRequestPrice, timePricingJSON, pricing.Platform,
		// sudoapi: Channel TTL-specific cache creation pricing.
		pricing.CacheCreation5mPrice, pricing.CacheCreation1hPrice, pricing.ID,
	)
	if err != nil {
...
func (r *channelRepository) batchLoadIntervals(ctx context.Context, pricingIDs []int64) (map[int64][]service.PricingInterval, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			// sudoapi: Channel TTL-specific cache creation pricing.
			&iv.CacheCreation5mPrice, &iv.CacheCreation1hPrice,
		); err != nil {
...
func scanModelPricingRows(rows *sql.Rows) ([]service.ChannelModelPricing, []int64, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			// sudoapi: Channel TTL-specific cache creation pricing.
			&p.CacheCreation5mPrice, &p.CacheCreation1hPrice,
		); err != nil {
...
func createModelPricingExec(ctx context.Context, exec dbExec, pricing *service.ChannelModelPricing) error {
	...
	err = exec.QueryRowContext(ctx,
		`INSERT INTO channel_model_pricing (channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_read_price, image_input_price, image_output_price, per_request_price, time_pricing, cache_creation_5m_price, cache_creation_1h_price)
...
func createIntervalExec(ctx context.Context, exec dbExec, iv *service.PricingInterval) error {
	return exec.QueryRowContext(ctx,
		`INSERT INTO channel_pricing_intervals
		 (pricing_id, min_tokens, max_tokens, tier_label, input_price, output_price, cache_write_price, cache_read_price, per_request_price, sort_order, cache_creation_5m_price, cache_creation_1h_price)
