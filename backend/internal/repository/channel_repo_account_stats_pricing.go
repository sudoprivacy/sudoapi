...
func (r *channelRepository) batchLoadAccountStatsModelPricing(ctx context.Context, ruleIDs []int64) (map[int64][]service.ChannelModelPricing, error) {
	...
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, rule_id, platform, models, billing_mode, input_price, output_price,
		        cache_write_price, cache_creation_5m_price, cache_creation_1h_price, cache_read_price, image_output_price, per_request_price, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&p.CacheCreation5mPrice, &p.CacheCreation1hPrice, &p.CacheReadPrice,
...
func createAccountStatsModelPricingTx(ctx context.Context, tx *sql.Tx, ruleID int64, pricing *service.ChannelModelPricing) error {
	...
	err = tx.QueryRowContext(ctx,
		`INSERT INTO channel_account_stats_model_pricing (rule_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_creation_5m_price, cache_creation_1h_price, cache_read_price, image_output_price, per_request_price)
