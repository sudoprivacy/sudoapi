...
func defaultClaudeOAuthSystemPromptBlockConfig(ttl string) []claudeOAuthSystemPromptBlockConfig {
	enabled := true
	if ttl == "" {
		ttl = claude.DefaultCacheControlTTL
	}
...
func inferClaudeOAuthSystemPromptCacheTTL(body []byte) string {
	if jsonValueContainsCacheTTL(gjson.ParseBytes(body), cacheTTLTarget1h) {
		return cacheTTLTarget1h
	}
	return claude.DefaultCacheControlTTL
}

func jsonValueContainsCacheTTL(value gjson.Result, ttl string) bool {
	if !value.Exists() {
		return false
	}
	if value.IsObject() {
		if cc := value.Get("cache_control"); cc.Exists() && cc.Get("type").String() == "ephemeral" && cc.Get("ttl").String() == ttl {
			return true
		}
		found := false
		value.ForEach(func(_, child gjson.Result) bool {
			if jsonValueContainsCacheTTL(child, ttl) {
				found = true
				return false
			}
			return true
...
func buildClaudeOAuthSystemPromptBlocksJSON(body []byte, expansionPrompt string, blocksConfig string) ([][]byte, error) {
	...
	if len(blocks) == 0 {
		blocks = defaultClaudeOAuthSystemPromptBlockConfig(inferClaudeOAuthSystemPromptCacheTTL(body))
	}
