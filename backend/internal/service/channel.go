...
type ChannelModelPricing struct {
	...
	CacheCreation5mPrice *float64 // 5分钟缓存创建价格
	CacheCreation1hPrice *float64 // 1小时缓存创建价格
}
...
type PricingInterval struct {
	...
	// sudoapi: Channel TTL-specific cache creation pricing.
	CacheCreation5mPrice *float64 // token 模式：5分钟缓存创建价
	CacheCreation1hPrice *float64 // token 模式：1小时缓存创建价
}
