...
const roleAccountContributor = "account_contributor"
...
func normalizeUserRole(role, fallback string) (string, error) {
	...
	if role != RoleAdmin && role != RoleUser && role != roleAccountContributor {
		return "", fmt.Errorf("invalid role: %q (must be %s, %s or %s)", role, RoleAdmin, RoleUser, roleAccountContributor)
	}
	return role, nil
}
