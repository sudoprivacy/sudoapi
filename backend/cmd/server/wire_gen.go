...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	contributorAccountHandler := handler.NewContributorAccountHandler(adminService, accountTestService, oAuthService, openAIOAuthService)
	...
	modelEndpointConfigService := service.NewModelEndpointConfigService(settingRepository)
	modelMetadataRepository := repository.NewModelMetadataRepository(db)
	modelMetadataService := service.NewModelMetadataService(modelMetadataRepository, channelService, pricingService)
	modelSettingService := service.NewModelSettingService(configConfig)
	modelSquareService := service.ProvideModelSquareService(channelService, pricingService, modelMetadataService, modelEndpointConfigService, modelSettingService)
	channelHandler := admin.NewChannelHandler(channelService, billingService, modelEndpointConfigService, modelSquareService, pricingService)
	modelMetadataHandler := admin.NewModelMetadataHandler(modelMetadataService, modelSquareService)
	modelSettingHandler := admin.NewModelSettingHandler(modelSettingService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, modelMetadataHandler, modelSettingHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler)
	geminiOpenAICompatService := service.NewGeminiOpenAICompatService(geminiMessagesCompatService)
	usageRecordWorkerPool := service.NewUsageRecordWorkerPool(configConfig)
	userMsgQueueCache := repository.NewUserMsgQueueCache(redisClient)
	userMessageQueueService := service.ProvideUserMessageQueueService(userMsgQueueCache, rpmCache, configConfig)
	gatewayHandler := handler.NewGatewayHandler(gatewayService, geminiMessagesCompatService, geminiOpenAICompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, contributorAccountHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelSquareHandler, idempotencyCoordinator, idempotencyCleanupService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService)
	apiKeyModelRouteResolver := service.NewAPIKeyModelRouteResolver(channelService)
	apiKeyAuthMiddleware := middleware.NewAPIKeyAuthMiddlewareWithRouteResolver(apiKeyService, subscriptionService, configConfig, apiKeyModelRouteResolver)
